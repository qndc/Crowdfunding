...
public class ProjectSettlement implements Job
{
  ...
  public void execute(JobExecutionContext context) throws JobExecutionException
  {
    ...
    AlipayConfig.logResult(now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))+":众筹时间到，审核开始...");
    AlipayConfig.logResult(now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))+":项目信息："+new Gson().toJson(project));
    AlipayConfig.logResult(now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))+":发起人信息："+new Gson().toJson(member));
//    try {
...
//				bean.setTotal_amount(tOrder.getMoney().toString());
...
//					AlipayConfig.logResult("退款成功，商户单号："+response.getOutTradeNo()+"，支付宝交易号："+response.getTradeNo());
